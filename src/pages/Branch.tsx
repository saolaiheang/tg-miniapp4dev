...
//     useEffect(() => {
//         fetch('http://localhost:3307/api/branch/all') 
//             .then(response => {
//                 if (!response.ok) {
...
const Branch = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:3307/api/branch/all') // Update URL as per your API endpoint
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
                }
    ...
    return (
        <div className="space-y-8">
            <div className='text-left mt-8'>
                <h1 className='text-3xl  font-bold font-jomhuria'>Branch & Contact</h1>
                <button className='mt-4 text-xl bg-green-600 text-white w-50 rounded-md' onClick={() => navigate("/workoutplan")}>Workout Plans</button>
            </div>
